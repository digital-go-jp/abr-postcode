...
Go 1.26 以降が必要。Docker で動かす場合は不要。
...
町字IDは市区町村の中でのみ一意なため、`/machiaza` は `lg_code` と組で指定する。

該当するデータがないときは 404、パラメータの書式が不正なときは 400 を返す。どちらも本文は `{"error": "..."}` になる。
...
[OpenAPI 仕様書](openapi/openapi.yml)
