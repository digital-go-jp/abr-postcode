...
const (
	// DefaultDCATFeedURL and DefaultDataURL come from the data package, which
	// owns where the ABR dataset lives.
	DefaultDCATFeedURL     = data.DefaultDCATFeedURL
	DefaultDataURL         = data.DefaultDataURL
	DefaultDataDir         = "data/"
	DefaultPort            = "8080"
...
func Load() *Config {
	return &Config{
		DCATFeedURL:      cmp.Or(os.Getenv("ABRP_FEED_URL"), DefaultDCATFeedURL),
		DataURL:          cmp.Or(os.Getenv("ABRP_DATA_URL"), DefaultDataURL),
